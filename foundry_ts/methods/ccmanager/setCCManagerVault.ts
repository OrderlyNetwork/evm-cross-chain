...
// Path: foundry_scripts/ccmanager/setCCManagerVault.s.sol
...
// current file name
const method_name = "setCCManagerVault";

export function setCCManagerVaultWithArgv(argv: any) {
    const required_flags = ["env", "network"];
    checkArgs(method_name, argv, required_flags);
    setCCManagerVault(argv.env, argv.network, argv.broadcast, argv.simulate);
}

export function setCCManagerVault(env: string, network: string, broadcast: boolean, simulate: boolean) {
...
addOperation(method_name, setCCManagerVaultWithArgv);
