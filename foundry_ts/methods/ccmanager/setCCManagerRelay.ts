...
// Path: foundry_scripts/ccmanager/setCCManagerRelay.s.sol
...
// current file name
const method_name = "setCCManagerRelay";

export function setCCManagerRelayWithArgv(argv: any) {
    const required_flags = ["env", "network"];
    checkArgs(method_name, argv, required_flags);
    setCCManagerRelay(argv.env, argv.network, argv.broadcast, argv.simulate);
}

export function setCCManagerRelay(env: string, network: string, broadcast: boolean, simulate: boolean) {
...
addOperation(method_name, setCCManagerRelayWithArgv);
