...
// Path: script/foundry_scripts/ccmanager/managerInitializeOnly.s.sol
...
// current file name
const method_name = "managerInitializeOnly";

export function managerInitializeOnlyWithArgv(argv: any) {
    const required_flags = ["env", "network", "role", "broadcast"];
    checkArgs(method_name, argv, required_flags);
    managerInitializeOnly(argv.env, argv.network, argv.role, argv.broadcast, argv.simulate);
}

export function managerInitializeOnly(env: string, network: string, role: string, broadcast: boolean, simulate: boolean) {
...
addOperation(method_name, managerInitializeOnlyWithArgv);
