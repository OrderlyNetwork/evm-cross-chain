...
// Path: foundry_scripts/ccmanager/setCCManagerOperator.s.sol
...
// current file name
const method_name = "setCCManagerOperator";

export function setCCManagerOperatorWithArgv(argv: any) {
    const required_flags = ["env", "network"];
    checkArgs(method_name, argv, required_flags);
    setCCManagerOperator(argv.env, argv.network, argv.broadcast, argv.simulate);
}

export function setCCManagerOperator(env: string, network: string, broadcast: boolean, simulate: boolean) {
...
addOperation(method_name, setCCManagerOperatorWithArgv);
