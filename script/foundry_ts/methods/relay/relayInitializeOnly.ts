...
// Path: script/foundry_scripts/relay/relayInitializeOnly.s.sol
...
// current file name
const method_name = "relayInitializeOnly";

export function relayInitializeOnlyWithArgv(argv: any) {
    const required_flags = ["env", "network", "broadcast"];
    checkArgs(method_name, argv, required_flags);
    relayInitializeOnly(argv.env, argv.network, argv.broadcast, argv.simulate);
}

export function relayInitializeOnly(env: string, network: string, broadcast: boolean, simulate: boolean) {
...
addOperation(method_name, relayInitializeOnlyWithArgv);
