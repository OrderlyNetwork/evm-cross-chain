...
// Path: foundry_scripts/relay/sendPingPong.s.sol
...
// current file name
const method_name = "sendPingPong";

export function sendPingPongWithArgv(argv: any) {
    const required_flags = ["env", "network", "dstNetwork"];
    checkArgs(method_name, argv, required_flags);
    sendPingPong(argv.env, argv.network, argv.dstNetwork, argv.broadcast, argv.simulate);
}

export function sendPingPong(env: string, network: string, dstNetwork: string, broadcast: boolean, simulate: boolean) {
...
addOperation(method_name, sendPingPongWithArgv);
