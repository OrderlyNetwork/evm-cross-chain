...
contract DeployProxyOnly is BaseScript, ConfigHelper, CCManagerHelper {
    ...
    function run() external {
        string memory env = vm.envString("FS_deployProxyOnly_env");
        string memory network = vm.envString("FS_deployProxyOnly_network");
        ...
        address manager = managerData.manager;

        address proxy = deployProxyOnly(manager);
