...
contract ConfigHelper is Script {
    ...
    function getValueByKey(string memory path, string memory key1) internal view returns (bytes memory) {
        string memory fileData = vm.readFile(path);
        bytes memory encodedData = vm.parseJson(fileData, key1.formJsonKey());
        return encodedData;
    }
    ...
    function getUsdcDecimal()
        internal
        view
        returns (TokenDecimalConfig memory)
    {
        string memory tokenDecimalsConfigPath = vm.envString("TOKEN_DECIMAL_CONFIG_FILE");
        bytes memory encodedData = getValueByKey(tokenDecimalsConfigPath, "default");
        TokenDecimalConfig memory config = abi.decode(encodedData, (TokenDecimalConfig));
        return config;
    }
